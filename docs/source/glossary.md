...
```{glossary}
Manifest file
    A tabular file at {{fpath_manifest}} contains ground truth information about the participants and visits/sessions available for a dataset. There must be only one row per unique participant/visit combination. This file is typically generated and maintained by the dataset owner who is informed on the study design and data collection protocols. This file serves as a starting point for all other Nipoppy tasks and therefore must be validated and updated with care.

Global config file
    A JSON file at {{fpath_config}} contains the high-level description of a dataset (e.g. name, visit and session names) and configurations for running the available pipelines (e.g., pipeline version). This file typically needs to be customized to run data processing and imaging-derived-phenotype extraction tasks on the local dataset.
...
Pybids ignore file
    A JSON file at {{fpath_pybids_ignore}} that contains a list of file names or patterns to ignore when building the PyBIDS database. This is useful to customize processing runs that may need a specific subset of files in the BIDS dataset.

Tracker config file
    A JSON file at {{fpath_tracker_config}} file should include lists of paths to track (i.e. check if exist on disk). A path can include template strings for participant/session IDs and/or glob expressions
